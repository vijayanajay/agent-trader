...
def run_backtest(ticker: str, data_path: Path, results_path: Path):
    ...
    try:
        # Timestamps are parsed from the index.
    ...
    print(f"Backtest complete. {len(trades)} trades logged to {results_path}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run a deterministic backtest for a stock.")
    parser.add_argument(
        "--ticker",
        type=str,
